...
export const REALM_PRESETS = {
    cycle: {
        id: 'cycle',
        name: 'Dynamic Cycle',
        desc: 'Natural passage of daylight, dusk, and night',
        icon: '⏳',
        swatch: 'linear-gradient(135deg, #c4884a 0%, #6a8a5a 50%, #0a0a08 100%)'
    },
    day: {
        id: 'day',
        name: 'Daylight Valley',
        desc: 'Vibrant golden noon across ancient lands',
        icon: '☀️',
        fixedTime: 0.28,
        swatch: '#6a8a5a'
    },
    dusk: {
        id: 'dusk',
        name: 'Dusk & Ember',
        desc: 'Warm bronze and crimson sunset skies',
        icon: '🌅',
        fixedTime: 0.48,
        swatch: '#c45a2a'
    },
    night: {
        id: 'night',
        name: 'Sacred Night',
        desc: 'Moonlit starry realm with glowing fireflies',
        icon: '🌙',
        fixedTime: 0.78,
        swatch: '#0a0a08'
    }
...
export class DayNightCycle {
    constructor(scene, sunLight, fillLight, extras = {}) {
        ...
        // Saved realm background
        const savedRealm = localStorage.getItem('vasuki_background_realm');
        this.currentRealmId = (savedRealm && REALM_PRESETS[savedRealm]) ? savedRealm : 'cycle';
        if (this.currentRealmId !== 'cycle' && REALM_PRESETS[this.currentRealmId]) {
            this.timeOfDay = REALM_PRESETS[this.currentRealmId].fixedTime;
        }
        ...
        this.skyDawn = new THREE.Color(0xC4884A);
        this.skyDay = new THREE.Color(0x6A8A5A);
        this.skyDusk = new THREE.Color(0xA0684A);
        this.skyNight = new THREE.Color(0x0A0A08);

        this.horizonDawn = new THREE.Color(0xD4A060);
        this.horizonDay = new THREE.Color(0xB0C8A0);
        this.horizonDusk = new THREE.Color(0xC48050);
        this.horizonNight = new THREE.Color(0x1A1812);

        this.sunDawn = new THREE.Color(0xD4A060);
        this.sunDay = new THREE.Color(0xFFF0D4);
        this.sunDusk = new THREE.Color(0xC45A2A);
    ...
    setRealm(realmId) {
        if (!REALM_PRESETS[realmId]) realmId = 'cycle';
        this.currentRealmId = realmId;
        localStorage.setItem('vasuki_background_realm', realmId);
        if (realmId !== 'cycle' && REALM_PRESETS[realmId]) {
            this.timeOfDay = REALM_PRESETS[realmId].fixedTime;
        }
    }

    update(delta, cameraPosition) {
        if (this.currentRealmId === 'cycle') {
            this.timeOfDay = (this.timeOfDay + delta / this.cycleDuration) % 1.0;
        } else if (REALM_PRESETS[this.currentRealmId]) {
            this.timeOfDay = REALM_PRESETS[this.currentRealmId].fixedTime;
        }
